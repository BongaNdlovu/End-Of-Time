...
function fetchAndDisplayLeaderboard() {
  try {
    ...
    const q = leaderboardCol.orderBy('score', 'desc').limit(100);
    ...
    .then(snapshot => {
      ...
      if (!foundCurrent && currentUser) {
        const allLeaderboardCol = db.collection('leaderboard');
        const allQuery = allLeaderboardCol.orderBy('score', 'desc').orderBy('time', 'asc');
...
function testFirebaseConnection() {
  ...
  // Test Firebase connection when the page loads - REMOVED
  // This should be called AFTER Firebase is initialized in DOMContentLoaded
  // testFirebaseConnection();
...
// 🔒 SECURE VERSION - Uses Cloud Function for server-side validation
async function updateLeaderboardScore(level, score) {
    ...
    try {
        ...
        // Prepare score data with additional validation info
        const scoreData = {
            score: score,
            level: level,
            displayName: currentUser.displayName || 'Anonymous',
            correctAnswers: correctAnswers, // From game state
            questionCount: gameQuestionCount, // From game state
            timeElapsed: gameElapsedTime, // From game state (in seconds)
            wagerTotal: 0, // Add if you have wager system
            powerUpsUsed: usedPowerUps.length || 0 // Track power-up usage
        };

        console.log('📤 Submitting score via Cloud Function:', scoreData);

        const result = await submitScore(scoreData);

        console.log('✅ Score submitted successfully:', result.data.message);

        // Show success feedback to user
        showScoreSubmissionFeedback('✅ Score saved successfully!', 'success');
        ...
        if (error.code === 'unauthenticated') {
            console.error('🔐 User not authenticated');
            showScoreSubmissionFeedback('❌ Please sign in to save your score.', 'error');
        } else if (error.code === 'resource-exhausted') {
            console.error('⏱️ Rate limit exceeded');
            showScoreSubmissionFeedback('⏱️ Please wait before submitting another score.', 'warning');
        } else if (error.code === 'invalid-argument') {
            console.error('📝 Invalid score data');
            showScoreSubmissionFeedback('❌ Invalid score detected. Please play fairly!', 'error');
        } else if (error.code === 'internal') {
            console.error('🌐 Server error');
            showScoreSubmissionFeedback('❌ Server error. Please try again later.', 'error');
        } else {
            console.error('🔴 Unknown error:', error.message);
            showScoreSubmissionFeedback('❌ Failed to save score. Please check your connection.', 'error');
        }
    }
}

// Helper function to show score submission feedback to user
function showScoreSubmissionFeedback(message, type) {
    // Check if there's an existing feedback element
    let feedbackEl = document.getElementById('score-submission-feedback');

    if (!feedbackEl) {
        // Create feedback element if it doesn't exist
        feedbackEl = document.createElement('div');
        feedbackEl.id = 'score-submission-feedback';
    ...
    if (type === 'success') {
        feedbackEl.style.background = '#4CAF50';
        feedbackEl.style.color = 'white';
    } else if (type === 'error') {
        feedbackEl.style.background = '#f44336';
        feedbackEl.style.color = 'white';
    } else if (type === 'warning') {
        feedbackEl.style.background = '#ff9800';
        feedbackEl.style.color = 'white';
    }
    ...
    db.collection('leaderboard')
      ...
      .then(querySnapshot => {
          ...
          querySnapshot.forEach((doc, index) => {
              ...
              const date = data.lastUpdated ? new Date(data.lastUpdated.seconds * 1000).toLocaleDateString() : 'N/A';

              const row = `
                  <tr style="background:${currentUser && currentUser.uid === doc.id ? 'rgba(139,0,0,0.3)' : 'transparent'};">
