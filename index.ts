...
export const onInteractionCreateV2 = onDocumentCreated('prayers/{prayerId}/interactions/{interactionId}', async (event) => {
  const interaction = event.data?.data() as any;
...
function getEmailBody(type: string, data: any = {}): string {
  switch (type) {
    case 'prayer_received':
...
function generatePrayerNotificationEmail(userData: any, prayer: any, interaction: any): string {
...
function generateCommentNotificationEmail(userData: any, prayer: any, interaction: any): string {
  const userName = userData.displayName || (userData.email ? userData.email.split('@')[0] : 'Friend');
...
export const sendEmailNotificationV2 = onDocumentCreated('emailQueue/{docId}', async (event) => {
...
function getWeeklyVerse(): { text: string; reference: string } {
  const verses = [
...
async function gatherWeeklySummaryData(userId: string, since: Date): Promise<any> {
  const summaryData = {
    hasActivity: false,
    userPrayers: [] as any[],
    prayersReceived: 0,
    commentsReceived: [] as any[],
    answeredPrayers: [] as any[],
    communityHighlights: [] as any[],
...
function generateWeeklySummaryEmail(userData: any, summaryData: any): string {
  const userName = userData.displayName || (userData.email ? userData.email.split('@')[0] : 'Friend');
  return `
<!DOCTYPE html>
<html>
...
export const sendWeeklySummariesV2 = onSchedule({
  schedule: '0 17 * * 6',
...
export const triggerWeeklySummaryManuallyV2 = onRequest(async (req, res) => {
...
export const sendEmailNotification = firestore
...
export const sendWeeklySummaries = pubsub
  .schedule('0 17 * * 6')
  .timeZone('Africa/Johannesburg')
  .onRun(async () => {
...
export const triggerWeeklySummaryManually = https.onRequest(async (req, res) => {
  if (process.env.NODE_ENV === 'production' && req.headers.authorization !== `Bearer ${process.env.ADMIN_KEY}`) {
    res.status(403).send('Unauthorized');
    return;
  }
